...
function gameLoop() {
  ...
  if (
    parseInt(snake[0][0].style.left) == candy_location[0] &&
    parseInt(snake[0][0].style.top) == candy_location[1]
  ) {
    ...
    snake.push([
      createSnakeBlock(
        parseInt(last_snake[0].style.top) / grid + addToTop,
        parseInt(last_snake[0].style.left) / grid + addToLeft
      ),
      last_snake[1],
    ]);
  } else {
    for (var i = 1; i < snake.length; i++) {
      if (
        parseInt(snake[0][0].style.left) == parseInt(snake[i][0].style.left) &&
        parseInt(snake[0][0].style.top) == parseInt(snake[i][0].style.top)
      ) {
        game_run = false;
        alert("Game Over");
      }
  ...
  for (var i = 0; i < change_dir.length; i++) {
    ...
    if (change_dir[i] < snake.length) {
      ...
      delete change_dir[i];
      change_dir.filter(function (val) {
        return val;
      });
    }
  }
